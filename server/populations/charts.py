...
def intra_decile_graph_data(baseline, reformed):
    ...
    gain = reformed.calc("household_net_income", map_to="person") - baseline.calc("household_net_income", map_to="person")
    ...
    for upper, lower, name in zip(bands[:-1], bands[1:], NAMES):
        ...
        tmp = pd.DataFrame({"fraction": fractions, "decile": list(map(str, range(1, 11))), "Outcome": name})
        l.append(tmp)
        subset = rel_gain
        if lower is not None:
            subset = subset[rel_gain > lower]
        if upper is not None:
            subset = subset[rel_gain <= upper]
        all_row = pd.DataFrame({"fraction": [subset.count() / rel_gain.count()], "decile": "All", "Outcome": name})
        l.append(all_row)
    return pd.concat(l).reset_index()

INTRA_DECILE_COLORS = ("#616161", GRAY, "#F5F5F5", "#C5E1A5", "#558B2F",)[::-1]

def intra_decile_chart(baseline, reformed):
    df = intra_decile_graph_data(baseline, reformed)
    fig1 = px.bar(df[df.decile != 'All'], x="fraction", y="decile", color="Outcome", color_discrete_sequence=INTRA_DECILE_COLORS, orientation='h')
    fig2 = px.bar(df[df.decile == 'All'], x="fraction", y="decile", color="Outcome", color_discrete_sequence=INTRA_DECILE_COLORS, orientation='h')
    fig = make_subplots(
        rows=2, 
        cols=1, 
        shared_xaxes=True, 
