...
import {
	Menu, PageHeader, Divider, Button, Empty, Spin, Steps, Statistic, Card,
} from "antd";
					...
					<div className="d-flex justify-content-center">
						<div style={{ width: 1000 }}>
							<Steps current={props.step}>
								<Step title="Policy" description="Specify changes to the current taxes and benefit programmes" />
...
class App extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
			selected: "main_rates", plan: DEFAULT_PLAN, results: null, ready: false, waiting: true,
		};
							...
							<Row>
								<Col md={3} style={{ paddingRight: 0 }}>
									<div className="main-menu">
										<ControlTab onClick={(name) => {
											this.setState({ selected: name.key });
										}}
										/>
									</div>
								</Col>
								<Col md={6} style={{ padding: 50 }}>
									<div className="main-menu">
										<Controls
											...
											onChange={(name, val) => {
												const { plan } = this.state; plan[name].value = val; this.setState({ plan });
											}}
										/>
									</div>
								</Col>
...
function SimulateButton(props) {
	...
	const url = `/simulate?${searchParams.toString()}`;
	return (
		<div>
